...
namespace Qlibrary
{
    public abstract class SplayTreeBase<T>
    {
        ...
        public LinkCutNode<T> MyNew(T key)
        {
            return new LinkCutNode<T>(key);
        }
        ...
        {
            if (l == r) return null;
            return l + 1 == r ? MyNew(values[l]) : Merge(Build(l, (l + r) / 2, values), Build((l + r) / 2, r, values));
        }
        ...
        public void Insert(ref LinkCutNode<T> node, int k, T key)
        {
            ...
            node = Merge(Merge(left, MyNew(key)), right);
        }
