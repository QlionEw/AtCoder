...
namespace Qlibrary
{
    public static class CycleDetection
    {
        ...
        public static List<(int, int)> DetectCycleOf<T>(this Graph<T> g, int x)
            where T : INumber<T> => DetectCycles(g, x, x + 1).FirstOrDefault() ?? new List<(int, int)>();
        
        /// <summary> グラフのループをどれか一つ検出 </summary>
        public static List<(int, int)> DetectCycle<T>(this Graph<T> g)
            where T : INumber<T> => DetectCycles(g, 0, g.Count).FirstOrDefault() ?? new List<(int, int)>();
        ...
        /// 1-2-3 , 1-4-3 とかは無理
        /// </summary>
        public static IEnumerable<List<(int, int)>> DetectCycles<T>(this Graph<T> g)
            where T : INumber<T> => DetectCycles(g, 0, g.Count);
        
        private static IEnumerable<List<(int, int)>> DetectCycles<T>(this Graph<T> g, int start, int end)
            where T : INumber<T>
        {
